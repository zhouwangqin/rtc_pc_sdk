...
bool ZXClient::SendPublish(std::string sdp, bool bAudio, bool bVideo, int audioType, int videoType)
{
	if (pZXEngine == nullptr)
	{
		return false;
...
void ZXClient::sendBroadcastCmd(std::string targetUid, int cmdType, std::string cmdData)
{
	if (pZXEngine == nullptr)
	{
		return;
